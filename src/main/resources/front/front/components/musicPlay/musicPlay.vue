<template>
  <view v-if="list.length" :style="{'bottom':bottomHeight + 'px'}" class="page">
    <view v-if="listShow" class="listBG" @click="listShow=false"></view>
    <view v-if="listShow" class="list">
      <scroll-view :scroll-y="true" style="max-height: 60vh;">
        <view v-for="(item,index) in list" :key="index" :class="musicIndex==index?'musicActive':''" class="music"
              @click="playListClick(index)">
          <view v-if="musicIndex!=index" class="left">
            {{ index + 1 }}
          </view>
          <view v-if="musicIndex==index" class="left">
            <image :src="item.tupian?baseUrl + item.tupian.split(',')[0]:'/static/player/plays.jpg'"
          ...
          <view class="right">
            <image mode="widthFix" src="/static/player/shanchu.png" @click.stop="delListClick(index)"></image>
          </view>
        </view>
      </scroll-view>
    </view>
    <view class="playView">
      ...
      <view class="playRight">
        <view class="top">
          ...
          <view class="play">
            <image v-if="!playType" class="bofang" mode="widthFix" src="/static/player/bofang.png"
                   @click="playsClick"></image>
            <image v-if="playType" class="bofang" mode="widthFix" src="/static/player/zanting.png"
                   @click="pauseClick"></image>
            <image v-if="playIndex==1" class="bofang1" mode="widthFix"
                   src="/static/player/danqu.png" @click="playIndexClick(2)"></image>
            <image v-if="playIndex==2" class="bofang1" mode="widthFix"
                   src="/static/player/xunhuan.png" @click="playIndexClick(3)"></image>
            <image v-if="playIndex==3" class="bofang1" mode="widthFix"
                   src="/static/player/suiji.png" @click="playIndexClick(1)"></image>
            <image class="bofang2" mode="widthFix" src="/static/player/list.png" @click="listClick"></image>
          </view>
        </view>

        <view style="display: flex;align-items: center;">
          <slider :block-size="12" :max="100" :min="0" :value="baifenbi" block-color="#ff0000"
                  style="width: 100%;" @change="sliderChange"/>
