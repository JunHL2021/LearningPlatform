  ...
  <view
    ...
    <view class="zb-table-content" style="flex: 1">
      <view class="zb-table-scroll" style="height: 100%;">
                ...
                <view class="zb-table-thead" style="position: relative;">
                  <view class="item-tr">
                    <view
                        ...
                        class="item-th" @click.stop="sortAction(item,index)">
                      <template v-if="item.type==='selection'">
                        <view class="checkbox-item">
                          <tableCheckbox
                              :checked="checkedAll" :indeterminate="indeterminate"
                      ...
                      <template v-else>
                        ...
                        <view v-if="item.sorter" class="sorter-table">
                          <view
                              :class="['sorter-table-icon',item.sorterMode==='_asc'&&`sorting${item.sorterMode||''}`]"></view>
                          <view
        ...
        <scroll-view
            id="tableBody" ref="tableBody" :lower-threshold="40" :scroll-left="bodyTableLeft"
            :scroll-top="bodyScrollTop"
            :style=" `height: calc(100% - ${showSummary?80:40}px)`"
            :upper-threshold="10"
            class="zb-table-body"
            scroll-x="true"
            scroll-y="true" @scroll="handleBodyScroll" @scrolltolower="scrolltolower"
            @scrolltoupper="(e)=>debounce(scrollToLeft)(e)">
          <view class="zb-table-fixed">
            <view class="zb-table-tbody">
              <view v-for="(item,index) in transData"
                    :key="item.key"
                    :class="['item-tr',highlight&&isHighlight(item,index)?'current-row':'']"
                ...
                <view
                    ...
                    <view style="display: flex;align-items: center;height: 100%">
                      <view
                        ...
                        <template v-else>
                          <button
                              :class="ren.class||''"
                              :size="ren.size||'mini'" :type="ren.type||'primary'">{{ ren.name }}
                          </button>
                        </template>
                      </view>
                    </view>
                  ...
                  <template v-else-if="ite.type==='selection'">
                    <view class="checkbox-item">
                      <tableCheckbox :cellData="item" :checked="item.checked"
                                     @checkboxSelected="(e)=>checkboxSelected(e,item)"/>
                    </view>
                  </template>
                    ...
                    <view class="checkbox-item">
                      <template v-if="item[ite.name]">
                        <image
                            ...
                            :src="iImageTem"
                            mode="aspectFit" style="width: 40px;height:30px; "
                            @click.stop="previewImage(item,item[ite.name],iImage)"></image>
                      </template>
            ...
            <view class="zb-table-tbody">
              <view
                ...
                <view v-for="(item,index) in fixedLeftColumns"
                  ...
                  <template v-if="item.type==='selection'">
                    <view class="checkbox-item">
                      <tableCheckbox :cellData="ite" :checked="ite.checked"
                                     @checkboxSelected="(e)=>checkboxSelected(e,ite)"/>
                    </view>
                  </template>
                  ...
                  <template v-if="item[ite.name]">
                    <view class="checkbox-item" @click.stop>
                      <image
                          ...
                          :src="iImageTem"
                          mode="aspectFit" style="width: 40px;height:30px; "
                          @click.stop="previewImage(iImageTem,item[ite.name],iImage)"></image>
                    </view>
                  </template>
