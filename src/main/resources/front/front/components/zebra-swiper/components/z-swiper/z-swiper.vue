<template>
  <view :id="'swiper'+_uid"
    ...
    <view :class="['swiper-wrapper']" :style="[wrapperStyle]" @click="onClickWrapper" @touchcancel="onTouchEnd"
          @touchend="onTouchEnd" @touchmove="onTouchMove" @touchstart="onTouchStart">
      <!-- #endif -->
      <!-- #ifdef MP-WEIXIN || MP-QQ -->
      <view :change:swiperTransform="zSwiperWxs.wxsTransformObserver" :class="['swiper-wrapper']"
            :style="[wrapperStyle]"
            :swiperTransform="wxsTransform" @click="onClickWrapper"
            @touchcancel="zSwiperWxs.onTouchEndWxs" @touchend="zSwiperWxs.onTouchEndWxs"
            @touchmove="zSwiperWxs.onTouchMoveWxs" @touchstart="zSwiperWxs.onTouchStartWxs">
        <!-- #endif -->
        <slot></slot>
        ...
        <template v-if="cubeShadowShowWrapper">
          <view :style="[cubeShadowStyle]" class="swiper-cube-shadow"></view>
        </template>
      </view>
      ...
      <template v-if="showIndicators">
        <view :class="['swiper-pagination',paginationClass]" :style="[paginationStyle]">
          ...
          <template v-if="paginationType == 'fraction'">
            <text :class="paginationContent.currentClass">{{ paginationContent.text }}</text>
            /
            <text
                :class="paginationContent.totalClass">{{ paginationContent.total }}
            </text>
          </template>
      ...
      <template v-if="scrollbarShow">
        <view :class="['swiper-scrollbar',scrollbarClass]" :style="[scrollbarStyle]"
          ...
          <view :style="[scrollbarItemStyle]" class="swiper-scrollbar-drag">
...
<!-- #ifdef MP-WEIXIN || MP-QQ -->
<script lang="wxs" module="zSwiperWxs" src="../../wxs/z-swiper-wxs.wxs"></script>
<!-- #endif -->
<script>
import {getRect} from '../../libs/utils/utils.js';
// vue2
import {getParams} from '../../libs/vue2/get-params.js';
import {initSwiper, mountSwiper} from '../../libs/vue2/init-swiper.js';
import {extend, needsScrollbar,} from '../../libs/vue2/utils.js';
import {calcLoopedSlides, renderLoop} from '../../libs/vue2/loop.js';
import {getChangedParams} from '../../libs/vue2/get-changed-params.js';
import {updateSwiper} from '../../libs/vue2/update-swiper.js';
import {updateOnVirtualData} from '../../libs/vue2/virtual.js';
//mixin
import {ParentMixin} from '../../libs/mixins/relation.js';
