<template>
  <view class="timetable">
    <view class="header">
      <view v-for="(item,index) in week" :key="item" :style="{ color: todayWeekIndex === index ? '#4070FF' : 'unset' }"
    ...
    <view class="main">
      ...
      <view class="course-container">
        <view v-for="(week, weekIndex) in courseData" :key="weekIndex" class="week">
          <view v-for="(course, courseIndex) in week" :key="courseIndex" class="courseList">
            <view v-if="course.length > 0"
                  :style="{ height: (course.length * 150) + 'px', background: course.backgroundColor }"
                  class="course"
                  @click="handleCourseClick(course, weekIndex, courseIndex)">{{ course.name }}
