<template>
  <view class="uni-rate">
    <view v-for="(star, index) in stars" :key="index" :style="{ marginLeft: margin + 'px' }" class="uni-rate__icon"
          @click="_onClick(index)">
      <uni-icons :color="color" :size="size" :type="isFill ? 'star-filled' : 'star'"/>
      <!-- #ifdef APP-NVUE -->
      <view :style="{ width: star.activeWitch.replace('%','')*size/100+'px'}" class="uni-rate__icon-on">
        <uni-icons :color="activeColor" :size="size" style="text-align: left;" type="star-filled"/>
      </view>
      <!-- #endif -->
      <!-- #ifndef APP-NVUE -->
