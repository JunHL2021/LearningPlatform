<template>
  <view class="uni-load-more" @click="onClick">
    <!-- #ifdef APP-NVUE -->
    <loading-indicator v-if="status === 'loading' && showIcon"
                       :animating="true" :style="{color: color,width:iconSize+'px',height:iconSize+'px'}"
                       class="uni-load-more__img uni-load-more__img--nvue"></loading-indicator>
    <!-- #endif -->
    <!-- #ifdef H5 -->
    <svg v-if="(iconType==='circle' || iconType==='auto' && platform === 'android') && status === 'loading' && showIcon"
         :style="{width:iconSize+'px',height:iconSize+'px'}" class="uni-load-more__img uni-load-more__img--android-H5"
         height="24"
         viewBox="25 25 50 50" width="24">
      <circle :style="{color:color}" cx="50" cy="50" fill="none" r="20" stroke-width="3"></circle>
    </svg>
    <!-- #endif -->
    <!-- #ifndef APP-NVUE || H5 -->
    <view
        v-if="(iconType==='circle' || iconType==='auto' && platform === 'android') && status === 'loading' && showIcon"
    ...
    <!-- #endif -->
    <!-- #ifndef APP-NVUE -->
    <view v-else-if="status === 'loading' && showIcon" :style="{width:iconSize+'px',height:iconSize+'px'}"
          class="uni-load-more__img uni-load-more__img--ios-H5">
      <view v-for="i in 12" :key="i"
            :style="{backgroundColor:color,height:iconSize/4+'px',transformOrigin:'1px '+iconSize/2+'px','-webkit-TransformOrigin':'1px '+iconSize/2+'px'}"></view>
    </view>
    <!-- #endif -->
    <text :style="{color: color}" class="uni-load-more__text">{{
        status === 'more' ? contentText.contentdown : status
        === 'loading' ? contentText.contentrefresh : contentText.contentnomore
      }}
    </text>
  </view>
</template>
