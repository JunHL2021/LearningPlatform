    ...
    <view v-if="leftBottom||rightBottom||leftTop||rightTop" :class="{
      ...
      <view :class="{
        ...
        <view v-for="(item, index) in content" :key="index" :class="{ 'uni-fab__item--active': isShow }"
          ...
          <image :src="item.active ? item.selectedIconPath : item.iconPath" class="uni-fab__item-image"
                 mode="widthFix"/>
          <text :style="{ color: item.active ? styles.selectedColor : styles.color }" class="uni-fab__item-text">{{
              item.text
            }}
          </text>
        </view>
      ...
      <view :class="{
		  ...
		  'uni-fab__content--other-platform': !isAndroidNvue
            ...
            :style="{ 'background-color': styles.buttonColor }" class="uni-fab__circle uni-fab__plus" @click="_onClick">
