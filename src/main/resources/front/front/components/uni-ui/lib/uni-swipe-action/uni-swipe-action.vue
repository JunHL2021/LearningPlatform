  ...
  <view class="uni-swipe">
    ...
    <view class="uni-swipe_content">
      <view ref="selector-button-hock" class="uni-swipe_button-group selector-query-hock move-hock">
        <view v-for="(item,index) in options" :key="index" :data-button="btn" :style="{
		    backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#C7C6CD',
		    fontSize: item.style && item.style.fontSize ? item.style.fontSize : '16px'
		  }"
              class="uni-swipe_button button-hock" @click.stop="onClick(index,item)">
          <text :style="{color: item.style && item.style.color ? item.style.color : '#FFFFFF',}"
                class="uni-swipe_button-text">{{ item.text }}
          </text>
        </view>
      </view>
      <view ref='selector-content-hock' class="selector-query-hock" @touchend="touchend" @touchmove="touchmove"
    ...
    <view class="uni-swipe_content">
      <view :change:prop="swipe.sizeReady" :data-disabled="disabled" :data-position="pos" :prop="pos"
            class="uni-swipe_move-box selector-query-hock move-hock"
            @change="change" @touchend="swipe.touchend" @touchmove="swipe.touchmove" @touchstart="swipe.touchstart">
        <view class="uni-swipe_box">
          <slot/>
        </view>
        <view ref="selector-button-hock" class="uni-swipe_button-group selector-query-hock move-hock">
    ...
    <view ref="selector-box-hock" class="uni-swipe_content" @horizontalpan="touchstart" @touchend="touchend">
      <view ref="selector-button-hock" :style="{width:right+'px'}"
            class="uni-swipe_button-group selector-query-hock move-hock">
        <view v-for="(item,index) in options" :key="index" ref="button-hock" :style="{
		  backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#C7C6CD',
...
<script lang="wxs" module="swipe" src="./index.wxs"></script>
<script>
// #ifndef APP-PLUS|| MP-WEIXIN || H5
