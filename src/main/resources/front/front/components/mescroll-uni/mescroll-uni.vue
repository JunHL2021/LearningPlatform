<template>
  <view class="mescroll-uni-warp">
    <scroll-view :id="viewId" :class="{'mescroll-uni-fixed':fixed}" :enable-back-to-top="true"
                 :scroll-top="scrollTop"
                 :scroll-with-animation="scrollAnim" :scroll-y='scrollAble'
                 :style="{'padding-top':padTop,'padding-bottom':padBottom,'top':fixedTop,'bottom':fixedBottom}"
                 :throttle="mescroll.optUp.onScroll==null" class="mescroll-uni" @scroll="scroll"
                 @touchcancel="touchendEvent" @touchend="touchendEvent" @touchmove="touchmoveEvent"
                 @touchstart="touchstartEvent">
      <view :style="{'transform': translateY, 'transition': transition}">
        <!-- 下拉加载区域-->
        <view v-if="mescroll.optDown.use" class="mescroll-downwarp">
          <view class="downwarp-content">
            <view :class="{'mescroll-rotate':isDownLoading}" :style="{'transform':downRotate}"
                  class="downwarp-progress"></view>
        ...
        <!-- 空布局 -->
        <view v-if="isShowEmpty" :class="{'empty-fixed':optEmpty.fixed}"
              :style="{'z-index':optEmpty.zIndex,'top':optEmpty.top}"
              class="mescroll-empty">
          <image v-if="optEmpty.icon" :src="optEmpty.icon" class="empty-icon" mode="widthFix"/>
    ...
    <image v-if="mescroll.optUp.toTop.src" :class="{'mescroll-fade-in':isShowToTop}" :src="mescroll.optUp.toTop.src"
           class="mescroll-totop" mode="widthFix" @click="toTopClick"/>
  </view>
</template>
