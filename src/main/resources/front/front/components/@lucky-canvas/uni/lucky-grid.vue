<template>
  <view v-if="isShow" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }" class="lucky-box">
    <canvas
        id="lucky-grid"
    ...
    <!-- #ifdef APP-PLUS -->
    <view v-if="btnShow">
      <view v-for="(btn, index) in btns" :key="index" :style="{
        top: btn.top + 'px',
        left: btn.left + 'px',
        width: btn.width + 'px',
        height: btn.height + 'px',
      }" class="lucky-grid-btn" @click="toPlay(btn, index)"></view>
    </view>
    <!-- #endif -->
    <!-- #ifndef APP-PLUS -->
    <view v-if="btnShow">
      <cover-view v-for="(btn, index) in btns" :key="index" :style="{
        top: btn.top + 'px',
        left: btn.left + 'px',
        width: btn.width + 'px',
        height: btn.height + 'px',
      }" class="lucky-grid-btn" @click="toPlay(btn, index)"></cover-view>
    </view>
    <!-- #endif -->
    <!-- #ifndef H5 -->
    <view v-if="myLucky">
        ...
        <div v-for="(block, index) in blocks" :key="index">
          <div v-if="block.imgs">
            <div v-for="(img, i) in block.imgs" :key="i">
              <image :data-i="i" :data-index="index" :src="img.src" @load="e => imgBindload(e, 'blocks')"></image>
              <image :data-i="i" :data-index="index" :src="img.activeSrc"
                     @load="e => imgBindloadActive(e, 'blocks')"></image>
            </div>
          </div>
        </div>
      </div>
        ...
        <div v-for="(prize, index) in prizes" :key="index">
          <div v-if="prize.imgs">
            <div v-for="(img, i) in prize.imgs" :key="i">
              <image :data-i="i" :data-index="index" :src="img.src" @load="e => imgBindload(e, 'prizes')"></image>
              <image :data-i="i" :data-index="index" :src="img.activeSrc"
                     @load="e => imgBindloadActive(e, 'prizes')"></image>
            </div>
          </div>
        </div>
      </div>
      <div class="lucky-imgs">
        <div v-for="(btn, index) in buttons" :key="index">
          <div v-if="btn.imgs">
            <image v-for="(img, i) in btn.imgs" :key="i" :data-i="i" :data-index="index" :src="img.src"
                   @load="e => imgBindload(e, 'buttons')"></image>
          </div>
        </div>
      </div>
      <div class="lucky-imgs">
        <span v-if="button && button.imgs">
          <image v-for="(img, i) in button.imgs" :key="i" :data-i="i" :src="img.src"
                 @load="e => imgBindloadBtn(e, 'button')"></image>
        </span>
      </div>
    </view>
    <!-- #endif -->
...
<script>
import {changeUnits, getImage, resolveImage} from './utils.js'
