<template>
  <view v-if="isShow" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }" class="lucky-box">
    <canvas id="lucky-wheel" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }" canvas-id="lucky-wheel"
            type="2d"></canvas>
    <image v-if="imgSrc" :src="imgSrc" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }"
           @load="myLucky.clearCanvas()"></image>
    <!-- #ifdef APP-PLUS -->
    <view :style="{ width: btnWidth + 'px', height: btnHeight + 'px' }" class="lucky-wheel-btn" @click="toPlay">
    </view>
    <!-- #endif -->
    <!-- #ifndef APP-PLUS -->
    <cover-view :style="{ width: btnWidth + 'px', height: btnHeight + 'px' }" class="lucky-wheel-btn"
                @click="toPlay"></cover-view>
    <!-- #endif -->
    <!-- #ifndef H5 -->
    <view v-if="myLucky">
      <div class="lucky-imgs">
        <div v-for="(block, index) in blocks" :key="index">
          <div v-if="block.imgs">
            <image v-for="(img, i) in block.imgs" :key="i" :src="img.src"
                   @load="e => imgBindload(e, 'blocks', index, i)"></image>
          </div>
        </div>
      </div>
      <div class="lucky-imgs">
        <div v-for="(prize, index) in prizes" :key="index">
          <div v-if="prize.imgs">
            <image v-for="(img, i) in prize.imgs" :key="i" :src="img.src"
                   @load="e => imgBindload(e, 'prizes', index, i)"></image>
          </div>
        </div>
      </div>
      <div class="lucky-imgs">
        <div v-for="(btn, index) in buttons" :key="index">
          <div v-if="btn.imgs">
            <image v-for="(img, i) in btn.imgs" :key="i" :src="img.src"
                   @load="e => imgBindload(e, 'buttons', index, i)"></image>
          </div>
        </div>
      </div>
    </view>
