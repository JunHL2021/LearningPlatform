<template>
  <uni-popup ref="popup" :safe-area="safeArea" type="bottom" @change="popChange">
    <view v-if="showPopPicker" class="date-time-picker">
      <!-- <view class="date-time-mask" @click.stop="hide"></view> -->
      <view :class="{'safe-area-padding':!safeArea}" class="date-time-container" @click.stop="handleEvent">
        <view class="time-picker-tool">
          <view :class="[cancelButtonClass]" class="cancel-base" @click.stop="close">
            <text>{{cancelButtonText}}</text>
          </view>
          <view :class="[toolBarTitleClass]">
            <text>{{toolBarTitle}}</text>
          </view>
          <view :class="[confirmButtonClass]" class="confirm-base" @click.stop="confirm">
            <text>{{confirmButtonText}}</text>
          </view>
        </view>
        <view v-if="isShowRange" class="picker-tab">
          <view :style="[rangeBtnStyle,dateTab==1?rangeBtnActiveStyle:'']" class="picker-tab-item"
                @click="dateTabChange(1)"
          >{{pickerData.startTime}}
          </view>
          至
          <view :style="[rangeBtnStyle,dateTab==2?rangeBtnActiveStyle:'']" class="picker-tab-item"
                @click="dateTabChange(2)"
          >{{pickerData.endTime}}
          </view>
        </view>
        <picker-view :indicator-style="popupIndicatorStyleString" :value="dateTime" class="picker-view"
                     @change="dateTimePickerChange" @pickstart="dateTimePickerStart" @touchstart="dateTimePickerStart">
          <picker-view-column v-if='isShowYear' data-id='year'>
            <view v-for="(item,index) in years" :key="index" class="item">{{item}}年</view>
          </picker-view-column>
          <picker-view-column v-if='isShowMonth' data-id='month'>
            <view v-for="(item,index) in months" :key="index" class="item">{{item}}月</view>
          </picker-view-column>
          <picker-view-column v-if='isShowDay' data-id='day'>
            <view v-for="(item,index) in days" :key="index" class="item">{{item}}日</view>
          </picker-view-column>
          <picker-view-column v-if='isShowHour' data-id='hour'>
            <view v-for="(item,index) in hours" :key="index" class="item">{{item}}时</view>
          </picker-view-column>
          <picker-view-column v-if='isShowMinute' data-id='minute'>
            <view v-for="(item,index) in minutes" :key="index" class="item">{{item}}分</view>
          </picker-view-column>
          <picker-view-column v-if='isShowSecond' data-id='second'>
            <view v-for="(item,index) in seconds" :key="index" class="item">{{item}}秒</view>
          </picker-view-column>
          <picker-view-column v-if='isShowWeek' data-id='week'>
            <view v-for="(item,index) in weeks" :key="index" class="item">{{item}}</view>
          </picker-view-column>
          <picker-view-column v-if='isShowQuarter' data-id='quarter'>
            <view v-for="(item,index) in quarters" :key="index" class="item">{{item}}</view>
          </picker-view-column>
        </picker-view>
      </view>
    </view>
