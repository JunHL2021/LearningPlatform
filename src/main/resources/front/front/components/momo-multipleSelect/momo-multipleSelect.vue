<template>
  <view v-show="show" class="select-container" @touchmove.stop.prevent>
    <view :class="activeClass ? 'mask-show' : ''" class="mask" @tap="onCancel(true)"></view>
    <view :class="activeClass ? 'select-box-show' : ''" class="select-box">
      <view class="header">
        <text class="cancel" @tap="onCancel">{{cancelText}}</text>
        <view v-if="allShow" class="all" @tap="onAllToggle">
