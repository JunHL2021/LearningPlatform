  ...
  <view>
    <view ref="uni-rate" class="uni-rate">
      <view v-for="(star, index) in stars" :key="index"
            :class="{'uni-cursor-not-allowed': disabled}" :style="{ 'margin-right': marginNumber + 'px' }"
            class="uni-rate__icon"
            @mouseleave="mouseleave" @touchstart.stop="touchstart" @touchmove.stop="touchmove"
            @mousedown.stop="mousedown" @mousemove.stop="mousemove">
        <uni-icons :color="color" :size="size" :type="isFill ? 'star-filled' : 'star'"/>
        <!-- #ifdef APP-NVUE -->
        <view :style="{ width: star.activeWitch.replace('%','')*size/100+'px'}" class="uni-rate__icon-on">
          <uni-icons :color="disabled?'#ccc':activeColor" :size="size" style="text-align: left;"
                     type="star-filled"/>
        </view>
        <!-- #endif -->
        <!-- #ifndef APP-NVUE -->
