    ...
    <view
      ...
      <form
        ...
        <view
          ...
          <input v-model="ruleForm.buzhishijian"
                 :disabled="ro.buzhishijian"
                 :style='{"border":"2rpx solid #fff","padding":"0px 24rpx","margin":"0px","color":"#666","borderRadius":"8rpx","flex":"1","background":"rgba(255,255,255,.6)","fontSize":"28rpx","height":"80rpx"}'
                 placeholder="布置时间" @tap="toggleTab('buzhishijian')"></input>
        </view>
        <view
            :style='{"padding":"0px 24rpx","margin":"0 0 24rpx 0","borderColor":"#7D7D7D","alignItems":"center","borderWidth":"0 0 2rpx","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'
            class="" @tap="zuoyefujianTap">
          <view
              :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'
              class="title">作业附件
          </view>
          <input
              v-if="ruleForm.zuoyefujian"
              v-model="baseUrl+ruleForm.zuoyefujian"
              :style='{"border":"2rpx solid #fff","padding":"0px 24rpx","margin":"0px","color":"#666","borderRadius":"8rpx","flex":"1","background":"rgba(255,255,255,.6)","fontSize":"28rpx","height":"80rpx"}'
              placeholder="作业附件"></input>
          <image v-else
                 :style='{"width":"80rpx","borderRadius":"100%","objectFit":"cover","display":"block","height":"80rpx"}'
                 class="avator" mode="aspectFill" src="../../static/gen/upload.png"></image>
        </view>
      ...
      <w-picker ref="buzhishijian" :current="false" :hasSecond="false" mode="dateTime" step="1"
                themeColor="#333333" @confirm="buzhishijianConfirm"></w-picker>
    </view>
  </view>
</template>
...
export default {
  ...
  async onLoad(options) {
    this.ruleForm.buzhishijian = this.$utils.getCurDateTime();
    ...
    if (options.id) {
      ...
      // 获取信息
      res = await this.$api.info(`buzhizuoye`, this.ruleForm.id);
...
//跨表计算判断
      ...
      if (crossrefid && crossuserid) {
        ...
        let res = await this.$api.list(`buzhizuoye`, params);
        if (res.data.total >= crossoptnum) {
          ...
          //跨表计算
          if (this.ruleForm.id) {
            await this.$api.update(`buzhizuoye`, this.ruleForm);
          } else {
            await this.$api.add(`buzhizuoye`, this.ruleForm);
          }
