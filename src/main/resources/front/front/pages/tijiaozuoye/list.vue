<template>
  ...
  <mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @init="mescrollInit" @up="upCallback">
    <view class="content">
      <view
        ...
        <view
          ...
          <view
            ...
            <input
                v-model="searchForm.kechengmingcheng"
                ...
                placeholder="课程名称" type="text"></input>
          </view>
          <view
              v-if="queryIndex==1"
              :style='{"margin":"0 0px","borderColor":"#ff9593","borderRadius":"0","flex":"1","background":"#fff","borderWidth":"0","lineHeight":"80rpx","position":"relative","borderStyle":"solid","height":"80rpx"}'
          ...
          <view
            ...
            <input
                ...
                :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}'
                placeholder="学生姓名" type="text"></input>
          </view>
          <button
          ...
          <view
            ...
            <view v-for="(product,index) in list" :key="index"
              ...
              <view
                ...
                {{ product.kechengmingcheng }}
              </view>
              <!--
                        <image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"100%","height":"200rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.kechengfengmian)" :src="product.kechengfengmian.split(',')[0]"></image>
                        <image :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"100%","height":"200rpx","order":"-1"}' mode="aspectFill" class="uni-product-image" v-else :src="product.kechengfengmian?baseUrl+product.kechengfengmian.split(',')[0]:''"></image>
              -->
              ...
              <view :style='{"padding":"0 0px","margin":"0 10rpx 0 0"}'>
                ...
                <text :style='{"color":"#666","lineHeight":"1.5","fontSize":"24rpx"}'>{{ product.xueshengzhanghao }}
                </text>
              </view>
              <!-- #ifdef MP-WEIXIN -->
              <view
                  :style='{"width":"100%","padding":"20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
                <view v-if="(userid && isAuth('tijiaozuoye','修改')) || (!userid && isAuthFront('tijiaozuoye','修改'))"
                      :data-row="product"
                ...
                <view v-if="(userid && isAuth('tijiaozuoye','删除')) || (!userid && isAuthFront('tijiaozuoye','删除'))"
                      :data-row="product"
      ...
      <button
          v-if="userid && isAuth('tijiaozuoye','新增')"
      ...
      <button
          v-if="!userid && isAuthFront('tijiaozuoye','新增')"
          :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#0C77D5","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}'
          class="add-btn" @click="onAddTap()">新增
      </button>
    </view>
...
export default {
  ...
  methods: {
    ...
    async upCallback(mescroll) {
      ...
      let res = {}
      if (this.userid) {
        res = await this.$api.page(`tijiaozuoye`, params);
      } else {
        params['sfsh'] = '是';
        res = await this.$api.list(`tijiaozuoye`, params);
      }
    ...
    onDeleteTap(id) {
      ...
      uni.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            await _this.$api.del('tijiaozuoye', JSON.stringify([id]));
    ...
    async search() {
      ...
      if (this.searchForm.banji) {
        searchForm['banji'] = '%' + this.searchForm.banji + '%'
      }
      if (this.searchForm.xueshengxingming) {
        searchForm['xueshengxingming'] = '%' + this.searchForm.xueshengxingming + '%'
      }
      let res = {};
      if (this.userid) {
        res = await this.$api.page(`tijiaozuoye`, searchForm);
      } else {
        searchForm['sfsh'] = '是';
        res = await this.$api.list(`tijiaozuoye`, searchForm);
      }
      // 如果是第一页数据置空
