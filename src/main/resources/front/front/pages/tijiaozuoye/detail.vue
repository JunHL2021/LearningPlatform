<template>
  <view>
    <mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @init="mescrollInit" @up="upCallback">
      <view class="content">
        <view
            :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#F1F1F1","height":"auto","margin-top":"100px"}'
            class="container">
          ...
          <view
            ...
            <view
                :style='{"padding":"4rpx 24rpx 8rpx","margin":"0 auto","borderColor":"#000","borderWidth":"0 2rpx","background":"#FFF7DF","display":"flex","width":"calc(100% - 48rpx)","borderStyle":"solid","height":"auto"}'
                class="detail-list-item" @tap="download(detail.zuoyeneirong)">
              <view
                  :style='{"width":"auto","padding":"0 10rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333","textAlign":"left"}'
                  class="lable">
                作业内容：
            ...
            <view v-if="userid"
              ...
              <view v-if="detail.sfsh=='是'"
                    :style='{"padding":"0px","margin":"0px","color":"#666","textAlign":"left","flex":"1","width":"auto","lineHeight":"48rpx","fontSize":"28rpx"}'
                    class="text">
                通过
              </view>
              <view v-if="detail.sfsh=='否'"
                    :style='{"padding":"0px","margin":"0px","color":"#666","textAlign":"left","flex":"1","width":"auto","lineHeight":"48rpx","fontSize":"28rpx"}'
                    class="text">
                不通过
              </view>
              <view v-if="detail.sfsh=='待审核'"
                    :style='{"padding":"0px","margin":"0px","color":"#666","textAlign":"left","flex":"1","width":"auto","lineHeight":"48rpx","fontSize":"28rpx"}'
                    class="text">
                待审核
              </view>
            ...
            <view
              ...
              <button
                  v-if="userid&&isAuth('tijiaozuoye','评分')"
                  ...
                  @tap="onAcrossTap('zuoyepingfen','是','','','')">评分
              </button>
              <button
                  v-if="!userid&&isAuthFront('tijiaozuoye','评分')"
                  :style='{"border":"0","padding":"0 20rpx","margin":"0 3% 20rpx 0","color":"#fff","borderRadius":"8rpx","background":"#0C77D5","width":"auto","fontSize":"28rpx","minWidth":"21%","lineHeight":"80rpx","height":"80rpx"}'
                  class="cu-btn bg-brown round shadow-blur"
                  @tap="onAcrossTap('zuoyepingfen','是','','','')">评分
              </button>
              <button
                  v-if="userid&&isAuth('tijiaozuoye','审核')"
              ...
              <button
                  v-if="!userid&&isAuthFront('tijiaozuoye','审核')"
                  :style='{"border":"0","padding":"0 20rpx","margin":"0 3% 20rpx 0","color":"#fff","borderRadius":"8rpx","background":"#0C77D5","width":"auto","fontSize":"28rpx","minWidth":"20%","lineHeight":"80rpx","height":"80rpx"}'
                  @tap="onSHTap">审核
              </button>
        ...
        <!-- 确认完成弹窗 -->
        <uni-popup ref="popup" class="popup-content" type="center">
          <form class="popup-form"
                style="background: #fff;width: 300px;display: block;border-radius: 10px;padding: 20px;text-align: center;">
            <view class=" margin-top">
              <picker :range="sfshOptions" :value="sfshIndex" @change="sfshChange">
                <view class="uni-input">{{ detail.sfsh ? detail.sfsh : '选择审核结果' }}</view>
              </picker>
            </view>
            <view class="">
              <!-- <view class="title">审核回复</view> -->
              <textarea v-model="detail.shhf" placeholder="审核回复" style="width: 200px;"></textarea>
              <!-- <input v-model="douyinId" placeholder="审核回复" name="input"></input> -->
            </view>
            <view class="btn-content">
              <button class="cu-btn bg-cyan" style="margin: 0 10px" @tap="onCloseWinTap">取消</button>
              <button class="cu-btn bg-red" style="margin: 0 10px" @tap="onFinishTap">确认</button>
            </view>
          </form>
        </uni-popup>
      </view>
...
export default {
  ...
  async onShow(options) {
    ...
    if (crossCleanType) {
      ...
      res = await this.$api.info('tijiaozuoye', this.id);
  ...
  methods: {
    ...
    async init(type = 1) {
      ...
      let res = await this.$api.info('tijiaozuoye', this.id);
      this.detail = res.data;
      ...
      if (type == 2) {
        ...
        await this.$api.update('tijiaozuoye', this.detail)
      }
    },
