...
export default {
  ...
  async onShow(options) {
    ...
    if (crossCleanType) {
      ...
      res = await this.$api.info('kechengleixing', this.id);
      this.detail = res.data;
      this.title = this.detail.kechengleixing
    }
  },
  destroyed: function () {
  ...
  methods: {
    ...
    async init(type = 1) {
      ...
      let res = await this.$api.info('kechengleixing', this.id);
      this.detail = res.data;

      this.title = this.detail.kechengleixing
      // 轮播图片
      this.swiperList = this.detail.image ? this.detail.image.split(",") : [];


      if (type == 2) {
        this.detail.discussnum++
        await this.$api.update('kechengleixing', this.detail)
      }
    },
