    ...
    <view
      ...
      <form
        ...
        <view
          ...
          <input
              v-model="ruleForm.ziyuanleixing"
              :disabled="ro.ziyuanleixing"
        ...
        <view
          ...
          <image v-if="ruleForm.image"
                 :src="baseUrl+ruleForm.image.split(',')[0]"
...
export default {
  ...
  async onLoad(options) {
    ...
    if (options.id) {
      ...
      // 获取信息
      res = await this.$api.info(`ziyuanleixing`, this.ruleForm.id);
...
//跨表计算判断
      ...
      if (crossrefid && crossuserid) {
        ...
        let res = await this.$api.list(`ziyuanleixing`, params);
        if (res.data.total >= crossoptnum) {
          ...
          //跨表计算
          if (this.ruleForm.id) {
            await this.$api.update(`ziyuanleixing`, this.ruleForm);
          } else {
            await this.$api.add(`ziyuanleixing`, this.ruleForm);
          }
