<template>
  ...
  <mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @init="mescrollInit" @up="upCallback">
    <view class="content">
        ...
        <view
          ...
          <view
            ...
            <text
                ...
                class="icon iconfont "></text>
            <input
                v-model="searchForm.banjimingcheng"
                :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}'
                placeholder="班级名称" type="text"></input>
          </view>
          <button
        ...
        <view
          ...
          <view
            ...
            <view v-for="(product,index) in list" :key="index"
              ...
              <image
                  v-if="preHttp(product.banjitupian)"
                  :src="product.banjitupian.split(',')[0]"
              ...
              <image
                  v-else
                  :src="product.banjitupian?baseUrl+product.banjitupian.split(',')[0]:''"
                  :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"100%","height":"200rpx","order":"-1"}'
                  class="uni-product-image"
                  mode="aspectFill"></image>
              ...
              <view
                ...
                <view v-if="(userid && isAuth('banjixinxi','修改')) || (!userid && isAuthFront('banjixinxi','修改'))"
                      :data-row="product"
                ...
                <view v-if="(userid && isAuth('banjixinxi','删除')) || (!userid && isAuthFront('banjixinxi','删除'))"
                      :data-row="product"
      ...
      <button
          v-if="userid && isAuth('banjixinxi','新增')"
      ...
      <button
          v-if="!userid && isAuthFront('banjixinxi','新增')"
          :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#0C77D5","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}'
          class="add-btn" @click="onAddTap()">新增
      </button>
    </view>
...
export default {
  ...
  methods: {
    ...
    async upCallback(mescroll) {
      ...
      if (this.searchForm.banjihao) {
        params['banjihao'] = '%' + this.searchForm.banjihao + '%'
      }
      if (this.searchForm.banjimingcheng) {
        params['banjimingcheng'] = '%' + this.searchForm.banjimingcheng + '%'
      }
      let user = uni.getStorageSync("appUserid") ? JSON.parse(uni.getStorageSync('userSession')) : {}
      let res = {}
      if (this.userid) {
        res = await this.$api.page(`banjixinxi`, params);
      } else {
        res = await this.$api.list(`banjixinxi`, params);
      }
    ...
    onDeleteTap(id) {
      ...
      uni.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            await _this.$api.del('banjixinxi', JSON.stringify([id]));
    ...
    async search() {
      ...
      if (this.searchForm.banjihao) {
        searchForm['banjihao'] = '%' + this.searchForm.banjihao + '%'
      }
      if (this.searchForm.banjimingcheng) {
        searchForm['banjimingcheng'] = '%' + this.searchForm.banjimingcheng + '%'
      }
      let res = {};
      if (this.userid) {
        res = await this.$api.page(`banjixinxi`, searchForm);
      } else {
        res = await this.$api.list(`banjixinxi`, searchForm);
      }
      // 如果是第一页数据置空
