<template>
  ...
  <mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @init="mescrollInit" @up="upCallback">
    <view class="content">
      <view
        ...
        <view
          ...
          <view
            ...
            <input
                v-model="searchForm.ziyuanmingcheng"
                ...
                placeholder="资源名称" type="text"></input>
          </view>
          <view
              v-if="queryIndex==1"
              :style='{"margin":"0 0px","borderColor":"#ff9593","borderRadius":"0","flex":"1","background":"#fff","borderWidth":"0","lineHeight":"80rpx","position":"relative","borderStyle":"solid","height":"80rpx"}'
            ...
            <text
                ...
                class="icon iconfont "></text>
            <input
                v-model="searchForm.ziyuanleixing"
                :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}'
                placeholder="资源类型" type="text"></input>
          </view>
          <button
        ...
        <view
          ...
          <scroll-view
            ...
            <view v-for="(item,index) in categoryList" :key="index"
                  :class='categoryName === item.ziyuanleixing ? "active" : ""' class="tab"
                  @tap="categoryClick(item.ziyuanleixing)">
              {{item.ziyuanleixing}}
            </view>
          </scroll-view>
          <!-- 样式1 -->
          <view
              :style='{"border":"2rpx solid #000","padding":"16rpx 10rpx 0","margin":"40rpx 0 0","borderRadius":"8rpx","flexWrap":"wrap","background":"#FFF7DF","flex":"1","display":"flex","width":"calc(100% - 220rpx)","justifyContent":"space-between","height":"auto"}'
              class="uni-product-list">
            <view v-for="(product,index) in list" :key="index"
                  :style='{"border":"2rpx solid #000","padding":"10rpx","margin":"0 0 40rpx","borderRadius":"8rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"48%","height":"auto"}'
                  class="uni-product" @tap="onDetailTap(product)">
              <view
                  :style='{"padding":"4rpx 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'
                  class="uni-product-title">
                {{product.ziyuanmingcheng}}
              </view>
              <view
                  :style='{"padding":"4rpx 20rpx","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'
                  class="uni-product-title">
                {{product.ziyuanleixing}}
              </view>
              <image
                  v-if="preHttp(product.ziyuantupian)"
                  :src="product.ziyuantupian.split(',')[0]"
                  :style='{"padding":"0","margin":"0","objectFit":"cover","borderRadius":"8rpx","display":"block","width":"100%","height":"200rpx","order":"-1"}'
              ...
              <!-- #ifdef MP-WEIXIN -->
              <view
                  :style='{"width":"100%","padding":"20rpx 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
                <view
                    v-if="(userid && isAuth('kechengziyuan','修改')) || (!userid && isAuthFront('kechengziyuan','修改'))"
                    :data-row="product"
                    :style='{"padding":"4rpx","background":"#0C77D5","display":"flex"}' @tap.stop.proevent="onUpdate">
                  <text
                      :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'
                      class="cuIcon-edit"></text>
                  <text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改
                  </text>
                </view>
                <view
                    v-if="(userid && isAuth('kechengziyuan','删除')) || (!userid && isAuthFront('kechengziyuan','删除'))"
                    :data-row="product"
                    :style='{"padding":"4rpx","background":"#F6BE1E","display":"flex"}' @tap.stop.proevent="onDelete">
                  <text
                      :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'
                      class="cuIcon-delete"></text>
                  <text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除
                  </text>
                </view>
              </view>
              <!-- #endif -->
              ...
              <view
                ...
                <view
                    v-if="(userid && isAuth('kechengziyuan','修改')) || (!userid && isAuthFront('kechengziyuan','修改'))"
                    :style='{"padding":"4rpx","background":"#0C77D5","display":"flex"}'
                    @tap.stop.proevent="onUpdateTap(product)">
                  <text
                      :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'
                      class="cuIcon-edit"></text>
                  <text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改
                ...
                <view
                    v-if="(userid && isAuth('kechengziyuan','删除')) || (!userid && isAuthFront('kechengziyuan','删除'))"
                    :style='{"padding":"4rpx","background":"#F6BE1E","display":"flex"}'
                    @tap.stop.proevent="onDeleteTap(product.id)">
                  <text
                      :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'
                      class="cuIcon-delete"></text>
                  <text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除
      ...
      <button
          v-if="userid && isAuth('kechengziyuan','新增')"
      ...
      <button
          v-if="!userid && isAuthFront('kechengziyuan','新增')"
          :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#0C77D5","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}'
          class="add-btn" @click="onAddTap()">新增
      </button>
    </view>
...
export default {
  ...
  async onShow() {
    ...
    let res = {};
    if (this.userid) {
      res = await this.$api.page('ziyuanleixing', {page: 1, limit: 100});
    } else {
      res = await this.$api.list('ziyuanleixing', {page: 1, limit: 100});
    }
    res.data.list.splice(0, 0, {id: 0, ziyuanleixing: '全部'})
  ...
  methods: {
    ...
    async upCallback(mescroll) {
      ...
      if (this.categoryName != '全部') {
        params.ziyuanleixing = '%' + this.categoryName + '%'
      }
      if (this.searchForm.ziyuanmingcheng) {
        params['ziyuanmingcheng'] = '%' + this.searchForm.ziyuanmingcheng + '%'
      }
      if (this.searchForm.ziyuanleixing) {
        params['ziyuanleixing'] = '%' + this.searchForm.ziyuanleixing + '%'
      }
      let user = uni.getStorageSync("appUserid") ? JSON.parse(uni.getStorageSync('userSession')) : {}
      let res = {}
      if (this.userid) {
        res = await this.$api.page(`kechengziyuan`, params);
      } else {
        res = await this.$api.list(`kechengziyuan`, params);
      }
    ...
    onDeleteTap(id) {
      ...
      uni.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            await _this.$api.del('kechengziyuan', JSON.stringify([id]));
    ...
    async search() {
      ...
      if (this.searchForm.ziyuanmingcheng) {
        searchForm['ziyuanmingcheng'] = '%' + this.searchForm.ziyuanmingcheng + '%'
      }
      if (this.categoryName != '全部') {
        searchForm.ziyuanleixing = '%' + this.categoryName + '%'
      }
      if (this.searchForm.ziyuanleixing) {
        searchForm['ziyuanleixing'] = '%' + this.searchForm.ziyuanleixing + '%'
      }
      let res = {};
      if (this.userid) {
        res = await this.$api.page(`kechengziyuan`, searchForm);
      } else {
        res = await this.$api.list(`kechengziyuan`, searchForm);
      }
      // 如果是第一页数据置空
