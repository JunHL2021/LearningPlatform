<template>
  ...
  <mescroll-uni :down="downOption" :up="upOption" @down="downCallback" @init="mescrollInit" @up="upCallback">
    <view class="content">
        ...
        <view
          ...
          <view
            ...
            <input
                ...
                placeholder="名称" type="text"></input>
          </view>
          <button
          ...
          <view
            ...
            <view v-for="(product,index) in list" :key="index"
              ...
              <view
                ...
                <view v-if="(userid && isAuth('friend','修改')) || (!userid && isAuthFront('friend','修改'))"
                      :data-row="product"
                ...
                <view v-if="(userid && isAuth('friend','删除')) || (!userid && isAuthFront('friend','删除'))"
                      :data-row="product"
      ...
      <button
          v-if="!userid && isAuthFront('friend','新增')"
          :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#0C77D5","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}'
          class="add-btn" @click="onAddTap()">新增
      </button>
    </view>
...
export default {
  ...
  methods: {
    ...
    async upCallback(mescroll) {
      ...
      let res = {}
      if (this.userid) {
        res = await this.$api.page(`friend`, params);
      } else {
        res = await this.$api.list(`friend`, params);
      }
    ...
    onDeleteTap(id) {
      ...
      uni.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            await _this.$api.del('friend', JSON.stringify([id]));
    ...
    async search() {
      ...
      let res = {};
      if (this.userid) {
        res = await this.$api.page(`friend`, searchForm);
      } else {
        res = await this.$api.list(`friend`, searchForm);
      }
      // 如果是第一页数据置空
