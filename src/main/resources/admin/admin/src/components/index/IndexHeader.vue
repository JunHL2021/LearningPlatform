<template>
  <div class="navbar">
    <div :style='{"display":"block","order":"0"}' class="title">
      <el-image v-if="false"
      ...
      <span
          :style='{"padding":"0 0 0 12px","lineHeight":"auto","fontSize":"24px","color":"inherit","float":"left","fontWeight":"500"}'
    ...
    <el-dropdown
        :style='{"fontSize":"inherit","position":"absolute","right":"50px","color":"inherit","display":"inline-block"}'
        trigger="click"
        @command="handleCommand">
      <div :style='{"alignItems":"center","display":"flex"}' class="el-dropdown-link">
        <el-image v-if="user"
                  :src="avatar?this.$base.url + avatar : require('@/assets/img/avator.png')"
                  :style='{"boxShadow":"0px 0px 10px rgba(0,0,0,.45)","margin":"0 15px","objectFit":"cover","borderRadius":"100%","display":"inline-block","width":"64px","height":"64px"}'
                  fit="cover"></el-image>
        <span
            :style='{"color":"inherit","lineHeight":"32px","fontSize":"inherit"}'>{{ this.$storage.get('adminName') }}</span>
        <span :style='{"color":"inherit","margin":"0 0 0 5px","fontSize":"14px"}'
              class="icon iconfont icon-a-xiala2"></span>
      </div>
      <el-dropdown-menu slot="dropdown" class="top-el-dropdown-menu">
        <el-dropdown-item :command="''" class="item1">首页</el-dropdown-item>
        <el-dropdown-item :command="'center'" class="item2">我的信息</el-dropdown-item>
        <el-dropdown-item :command="'logout'" class="item4">退出登录</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
